...
const AddAddress = () => {
  ...
  const onSubmitHandler = async (e) => {
    e.preventDefault();
    try {
      // include userId if available (server prefers req.userId via cookie)
      const payload = { address };
      if (user?._id) payload.userId = user._id;

      // ensure zipcode is numeric (server expects Number)
      if (payload.address?.zipcode !== undefined) {
        payload.address.zipcode = Number(payload.address.zipcode);
      }
